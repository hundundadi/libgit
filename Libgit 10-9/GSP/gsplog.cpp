...
void GspLog::Log(QString prjName, QString username, QString password, QStringList fileList)
{
    g_userName = username;
    g_password = password;
    ...
    int error = 0;
    git_libgit2_init();
    error = git_repository_open_ext(&rep, ba_path.constData(), 0, nullptr);
    if (fileList.isEmpty()) {
        char *logArgv[1] = {"log"};
        lg2_log(rep, 1, logArgv);
    }
    else {
        char* logArgv[2] = {nullptr};
        logArgv[0] = "log";
        for (int index = 0; index < fileList.size(); index++) {
            QString tmpFile = fileList.at(index);
            QByteArray ba_file = tmpFile.toUtf8();
            logArgv[1] = ba_file.data();
            lg2_log(rep, 2, logArgv);
        }
    }
...
int GspLog::lg2_log(git_repository *repo, int argc, char *argv[])
{
...
void GspLog::push_rev(struct log_state *s, git_object *obj, int hide)
{
    hide = s->hide ^ hide;
...
int GspLog::add_revision(struct log_state *s, const char *revstr)
{
    git_revspec revs;
    int hide = 0;
...
void GspLog::set_sorting(struct log_state *s, unsigned int sort_mode)
{
...
void GspLog::print_time(const git_time *intime, const char *prefix)
{
    ...
    sprintf(buffer, "%s%s %c%02d%02d", prefix, out, sign, hours, minutes);
    g_commitLog.dateTime = QString(buffer);
}

/** Helper to print a commit object. */
void GspLog::print_commit(git_commit *commit, struct log_options *opts)
{
    ...
    g_commitLog.commit_sha = QString(buf);
    ...
    if ((count = (int)git_commit_parentcount(commit)) > 1) {
        for (i = 0; i < count; ++i) {
            ...
            g_commitLog.mergeFrom.append(QString(buf));
        }
    }

    if ((sig = git_commit_author(commit)) != NULL) {
        g_commitLog.author = QString(sig->name);
        g_commitLog.email = sig->email;
        print_time(&sig->when, "");//Date:
    }
    g_commitLog.commit_info.clear();
    for (scan = git_commit_message(commit); scan && *scan; ) {
        for (eol = scan; *eol && *eol != '\n'; ++eol) /* find eol */;
        g_commitLog.commit_info += QString(scan);
        scan = *eol ? eol + 1 : NULL;
    }
}

/** Helper to find how many files in a commit changed from its nth parent. */
int GspLog::match_with_parent(git_commit *commit, int i, git_diff_options *opts)
{
    git_commit *parent;
    git_tree *a, *b;
    git_diff *diff;
...
int GspLog::parse_options(struct log_state *s, struct log_options *opt, int argc, char **argv)
{
